...
-(void)connectionDidFinishLoading:(NSURLConnection *)__unused connection {        
    ...
    if (html.length <= 0) {
        [self _failedExtractingYouTubeURLWithError:[NSError errorWithDomain:kLBYouTubeViewErrorDomain code:1 userInfo:@{NSLocalizedDescriptionKey: @"Couldn't download the HTML source code. URL might be invalid."}]];
        return;
    }
    ...
    if (JSONStart != nil) {
        ...
        if (decodingError) {
            ...
            NSDictionary* video = JSONCode[@"content"][@"video"];
            NSString* streamURL = nil;
            NSString* streamURLKey = @"stream_url";
            
            if (self.highQuality) {
                streamURL = video[[NSString stringWithFormat:@"hq_%@", streamURLKey]];
                if (!streamURL) {
                    streamURL = video[streamURLKey];
                }
            }
            else {
                streamURL = video[streamURLKey];
            ...
            if (streamURL) {
                ...
                [self _failedExtractingYouTubeURLWithError:[NSError errorWithDomain:kLBYouTubeViewErrorDomain code:2 userInfo:@{NSLocalizedDescriptionKey: @"Couldn't find the stream URL."}]];
            }
        }
    }
    else {
        [self _failedExtractingYouTubeURLWithError:[NSError errorWithDomain:kLBYouTubeViewErrorDomain code:3 userInfo:@{NSLocalizedDescriptionKey: @"The JSON data could not be found."}]];
    }
