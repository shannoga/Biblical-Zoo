                               ...
                               if ([ical parseWithString:str error:&error]) {
                                   if(!error){
                                       ...
                                       for (CGICalendarObject *icalObj in [ical objects]) {
                                           ...
                                           [Event parseEventsFromArray:[icalObj events] forClendarName:name];
                                       }
...
- (void)viewDidLoad
{
    ...
    segmentedControl.tintColor = UIColorFromRGB(0xFF733B);
...
-(void)reloadTableViewAfterSegmentedControllerChange:(id)sender{
    ...
    NSString *presicateString;
    switch (sc.selectedSegmentIndex) {
        case 1:
            presicateString=@"Feeding";
            break;
        case 2:
            presicateString=@"Kids";
            break;
        case 3:
            presicateString=@"Music";
            break;
    }
    ...
    if (sc.selectedSegmentIndex != 0 && sc.selectedSegmentIndex != 4  ) {
        NSPredicate *newPredicate = [NSPredicate predicateWithFormat:@"typeString == %@",presicateString];
        [[[self fetchedResultsController] fetchRequest] setPredicate:newPredicate];
    }else if (sc.selectedSegmentIndex == 4) {
        NSPredicate *newPredicate = [NSPredicate predicateWithFormat:@"typeString == %@ OR typeString == %@ OR typeString== %@",@"Exhibition",@"Show",@"Workshop"];
        [[[self fetchedResultsController] fetchRequest] setPredicate:newPredicate];
    }else{
